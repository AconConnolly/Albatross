...
// Connect to MongoDB
mongoose
  .connect("mongodb://localhost:27017/albatross", {
...
const url =
  "https://w.cps.golf/CityCalgaryGolfReservations/(S(da5jiz0rgjllwu5y1thj0ijv))/Home/WidgetView";
app.get("/tee-time-data", async (req, res) => {
  try {
    const response = await axios.get(url);
    const $ = cheerio.load(response.data);
    const teeTimes = $(".tee-time-slot");
    const teeTimeData = teeTimes
      .map((index, element) => {
        const time = $(element).find(".time-slot").text().trim();
        const available =
          $(element).find(".status").text().trim() === "Available";
...
app.post("/make-reservations", async (req, res) => {
  ...
  try {
    ...
    //Changed the below page to the landing page for current user login
    await page.goto(
      "https://cityofcalgarygolf.cps.golf/onlineresweb/auth/verify-email"
    );
    await page.type("#login-email", email);
    // Password appears after email
    ...
    const availableTeetimes = await page.$$(
      '.tee-time-slot .status:has-text("Available")'
    );
